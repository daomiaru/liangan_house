<template>
	<view>
		<view class="header" v-bind:class="{'status':isH5Plus}">
			<view class="userinfo">
				<template v-if="!loginShow">
					<view @tap="Tologin" style="font-size: 40upx;font-weight: bold;color: #F7F7F7;">您还未登录，点击登录！</view>
				</template>
				<template v-if="loginShow">
					<view class="face">
					...
					<view class="info">
						<view class="username">{{userinfo.nikName}}</view>
						<view class="integral">手机：{{userinfo.phone}}</view>
					</view>
				</template>
			</view>
			<view class="setting">
		...
		<view class="orders" style="position: relative;top: -10rpx;">
			<view class="box">
		...
		<button style="background: #007AFF;color: #F8F8F8;" @tap="loginOut">退出登录</button>
	</view>
</template>
<script>
	import {
	...
	export default {
		...
		onShow() {
			this.loginShow = this.$store.state.hasLogin
		...
		methods: {
			...
			//用户点击列表项
			toPage(list_i, li_i) {
				if (this.severList[list_i][li_i].name == '成为房东') {
					uni.navigateTo({
						url: '../beingCustodian/beingCustodian'
					})
				} else {
					uni.showToast({
