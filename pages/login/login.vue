<template>
	<view class="login">
		<view class="content">
			...
			<!-- <wButton text="登 录" :rotate="isRotate" @tap="startLogin"></wButton> -->
			<button type="default" @click="startLogin" style="width: 70%;">登录</button>
			<!-- 其他登录 -->
		...
		methods: {
			...
			startLogin() {
				...
				uni.request({
					...
					method: "POST",
					success: (e) => { //登录成功后改变vuex的状态并推出登录页
			...
			...mapMutations(['login']), //mapMutations([login])=login(login){this.$store.commit('login',login)}
			login_weixin() {
				//微信登录
				// uni.showToast({
				// 	icon: 'none',
